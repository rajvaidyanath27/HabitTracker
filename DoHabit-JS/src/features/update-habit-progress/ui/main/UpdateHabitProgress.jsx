...
import { useHabitsStore, getTodayProgress } from '@entities/habit';
...
function UpdateHabitProgress({
  ...
  const handleUpdateProgress = isLongPress => {
  ...
  const attrs = useLongPress(() => handleUpdateProgress(true), {
    threshold: 300
  });
  ...
  return <div className={clsx(styles.wrapper, animation === 'completed' && styles.completed, animation === 'updated' && styles.updated)}>
			{frequency > 1 && <ProgressBar segmentCount={frequency} progress={progress} />}

			<Button style={{
      backgroundColor: isCompleted ? 'var(--habit-color-base)' : 'var(--habit-color-dark)'
    }} className={clsx(styles.button, frequency > 1 && styles.multiFrequency)} {...attrs} onClick={() => handleUpdateProgress()}>
				{percentage >= 100 ? <FaCheck /> : <strong>{percentage}%</strong>}
			</Button>
		</div>;
}
