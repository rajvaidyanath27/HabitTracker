...
const habitAhievementRules = {
  ...
  'ill-be-back': ({
    ...
    const archivedCount = habits.reduce((count, h) => count + (h.isArchived ? 1 : 0), 0);
  ...
  'overachiever-day': ({
    ...
    const createdTodayCount = habits.reduce((count, h) => count + (formatDate(new Date(h.createdAt)) === todayStr ? 1 : 0), 0);
  ...
  'time-traveler': ({
    ...
    const totalLateCompletions = habits.reduce((acc, h) => {
      ...
      const lateCount = h.completedDays.reduce((count, day) => count + (day.isCompYdayBtnUsed ? 1 : 0), 0);
  ...
  'accumulated-year': ({
    habits
  }) => habits.some(h => h.completedDays && h.completedDays.length >= 365),
  'perfect-year': ({
    habits
  }) => habits.some(h => {
