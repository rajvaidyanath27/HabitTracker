...
function NoteList(props) {
   ...
   */
  const scopeNotes = useMemo(() => notes.filter(n => habitId ? n.habitId === habitId : !n.habitId), [habitId, notes]);

  /**
   * Extracts a list of years to populate the timeline filter options.
   */
  const availableYears = useMemo(() => extractYearsFromTimeline(scopeNotes, {
    order: 'desc'
  }), [scopeNotes]);
  ...
  sortOrder === 'desc' ? taggedNotes.toReversed() : taggedNotes, [sortOrder, taggedNotes]);
  ...
  */
  ...
  return <div className={styles.container}>
			<div className={styles.toolbar}>
				<SegmentedControl options={['All', ...availableYears].map(v => ({
        value: String(v)
      }))} value={String(selectedYear)} onChange={v => {
        setActiveTag(null);
        setSelectedYear(v === 'All' ? 'All' : Number(v));
      }} />

				<TagButton isActive={Boolean(activeTag)} onClick={() => {
        if (activeTag) {
          setActiveTag(null);
        } else {
          openNoteTagsMenu({
            ...
            onSetTag: setActiveTag
				...
				<SortButton order={sortOrder} onClick={() => setSortOrder(prev => prev === 'asc' ? 'desc' : 'asc')} />
			</div>
          ...
          return <section key={`${groupKey}_${sortOrder}`}>
								<div className={styles.monthLabelWrapper} data-first-group={index === 0 ? 'true' : undefined}>
									<AnimatePresence mode='popLayout' propagate>
										<motion.small key={groupKey} className={styles.monthLabel} variants={monthLabelVariants} initial='initial' animate='animate' exit='exit'>
											{MONTHS[monthIndex]}
										</motion.small>
									</AnimatePresence>
								</div>
								...
								<ul className={styles.noteList}>
									<AnimatePresence mode='popLayout' propagate>
										{notes.map(note => <motion.li key={`${note.id}_${sortOrder}_${activeTag}`} // Reset layout animation state on list updates to avoid layout shifts
                variants={cardVariants} initial='initial' animate='animate' exit='exit' layout='position' whileTap={{
                  ...
                  transition: {
                    duration: 0.1
												...
												<NoteCard note={note} onCardClick={() => openNoteMenu({
                    note,
                    onEdit: openNoteForm
                  })} onTagClick={tag => {
                    onScrollTop({
			...
			{visibleNotes.length < yearNotes.length && <div ref={loadMoreRef} style={{
      height: '60px'
    }} />}
		</div>;
}
