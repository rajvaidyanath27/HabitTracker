...
function HabitList(props) {
  ...
  // 2. Render list
  return <ul className={clsx(styles.habitList, isArchive && styles.isArchive)}>
			<AnimatePresence initial={false}>
				{filteredHabits.map(habit => {
        const colorVariants = palette[habit.colorIndex] ?? palette[0];
        const habitStats = getHabitStats(habit);
        const headerAction = isArchive ? <RestoreHabit habitId={habit.id} /> : <UpdateHabitProgress habit={habit} />;
        const calendarProps = {
          colorVariants,
          ...
          getCompletedDates: days => getCompletedDatesSet(habit.completedDays, ...days)
        };
        const calendar = !isArchive ? settings.calendarView === 'compact' ? <CompactCalendar {...calendarProps} /> : <Calendar {...calendarProps} /> : undefined;
        return <motion.li key={habit.id} whileTap={{
          ...
          transition: {
            duration: 0.1
							...
							<HabitCard habit={habit} headerAction={headerAction} colorVariants={colorVariants} currentStreak={isArchive ? undefined : habitStats.currentStreak} content={calendar} onClick={e => !isArchive && openHabitMenu({
            habit,
            habitStats,
            colorVariants,
