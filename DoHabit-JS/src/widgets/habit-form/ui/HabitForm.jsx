...
function HabitForm({
  ...
  return <form className={styles.form} onSubmit={handleSubmit} onKeyDown={handlePressEnter}>
			<HabitTitleInput input={title} isDuplicate={isDuplicate} onChange={setTitle} />

			<HabitFrequencyField initialFrequency={currentHabit?.frequency} />

			{isEditMode && currentHabit && <HabitOrderField habits={habits} habit={currentHabit} />}

			<HabitColorPicker habits={habits} initialColorIndex={currentHabit?.colorIndex} />

			<HabitIconPicker habits={habits} initialIconTitle={currentHabit?.iconTitle} />

			<div className={styles.submitButtonWrapper}>
				{isEditMode && habitId && <HabitExtraActions habitId={habitId} onSuccess={() => navigate('/')} />}

				<Button type='submit' icon={<MdAddToPhotos />} className={styles.submitButton} disabled={title.trim().length === 0 || isDuplicate}>
