...
function StreakHistory(props) {
  ...
  // 2. Render list
  return <Card title='Streak History' description='Shows streaks of 2 days or more.' extra={<FaBinoculars style={{
    color: baseColor
  }} />}>
			<div className={styles.history}>
				<ul className={styles.streaks}>
					{streakList.map((s, i) => <li key={s.length + s.start + s.end} style={{
          borderColor: darkenedColor
				...
				{listLength < streaks.length && <div className={styles.buttonWrapper}>
						<Button variant='text' onClick={() => setListLength(curr => curr + 5)}>
							Show more
						</Button>
					</div>}
			</div>
