...
function NoteForm(props) {
  ...
  return <AnimatePresence initial={false}>
			{isOpen && <>
					<Overlay key='note-form-dialog' onClick={() => closeForm(isEditMode)} />

					{createPortal(<motion.form key='note-form' initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} exit={{
        ...
        duration: 0.2
      }} className={styles.form} action='submit' onSubmit={handleSubmitForm}>
							<TextareaAutosize name='note-input' id='note-input' minRows={1} maxRows={10} value={draftText} placeholder='Enter your note here...' autoFocus autoComplete='off' onChange={e => setDraftText(e.target.value)} onKeyDown={handleKeyDown} />

							<Button key='submit-note-form-button' type='submit' icon={<IoSend />} initial={{
          opacity: 0,
          scale: 0
        }} animate={{
          opacity: 1,
          ...
          duration: 0.2
        }} className={styles.actionButton} disabled={!draftText.trim()} />
						</motion.form>, document.body)}
				</>}
		</AnimatePresence>;
}
export { NoteForm };
