...
function Month(props) {
  ...
  const weekdays = showDayNames ? [...WEEKDAYS.slice(1), 'Sunday'].map((weekday, index) => {
    const isToday = (index + 1) % 7 === today.getDay();
    const isCurrentMonth = today.getMonth() === monthDate.getMonth();
    return <div key={weekday} style={{
      color: isToday && isCurrentMonth ? 'inherit' : softenedColor
    }} className={styles.weekday}>
					{weekday.slice(0, 2)}
				</div>;
  }) : null;
  return <div className={styles.month}>
			{showDayNames && <div className={styles.weekdaysGrid}>
					{weekdays}
				</div>}
