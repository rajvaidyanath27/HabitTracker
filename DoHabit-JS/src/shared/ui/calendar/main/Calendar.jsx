...
function Calendar(props) {
  ...
  const datesRange = getDatesRange(daysCount, {
    from: firstDay
  });
  const completedSet = getCompletedDates(datesRange);
  return <div style={{
    gridTemplateColumns: `repeat(${visibleMonthsCount}, 1fr)`
  }} className={styles.calendar}>
			{Array.from({
      length: visibleMonthsCount
    }).map((_, i) => {
      const monthDate = new Date(now.getFullYear(), now.getMonth() - i, 1);
      return <Month key={monthDate.toISOString()} today={now} monthDate={monthDate} completedSet={completedSet} colorVariants={colorVariants} highlightToday={highlightToday} showDayNames={showDayNames} showDayNumbers={showDayNumbers} />;
