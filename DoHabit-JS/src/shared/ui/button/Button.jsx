...
function Button(props) {
  ...
  const innerContent = <>
			{typeof icon === 'string' ? <img src={icon} alt='icon' /> : renderIcon(icon, iconProps)}
			{children}
  ...
  // Render as Link
  if (to) {
    return <MotionLink to={to} state={state} style={style} className={classes} whileTap={whileTap}>
				{innerContent}
			</MotionLink>;
  }

  // Render as native Button
  return <motion.button type={type} style={style} className={classes} whileTap={whileTap} {...rest}>
			{innerContent}
		</motion.button>;
}
export { Button };
