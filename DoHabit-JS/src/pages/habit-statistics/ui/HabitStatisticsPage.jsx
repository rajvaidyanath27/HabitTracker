...
function HabitStatisticsPage() {
  ...
  const earliestYear = new Date(completedDays.at(-1)?.date ?? String(currYear)).getFullYear();
  const [selectedYear, setSelectedYear] = useState(currYear);
  // --- Selected Year:END ---

  const selectedDays = completedDays.filter(day => new Date(day.date).getFullYear() === selectedYear);
  ...
  return <div className={styles.statistics}>
			<YearPicker value={selectedYear} min={earliestYear} max={currYear} onChange={setSelectedYear} />

			<StreakOverview currentStreak={currentStreak} longestStreak={longestStreak} color={baseColor} />

			<WeekdayChart days={selectedDays} options={chartOptions} color={baseColor} />

			<TotalCompletedMetric days={selectedDays} color={baseColor} />

			<MonthlyChart days={selectedDays} options={chartOptions} color={baseColor} />

			<StreakHistory streaks={filteredStreaks} colorVariants={colorVariants} />
		</div>;
}
