...
function editHabit(params) {
  ...
  let nextHabits = updateHabitById(habits, payload.habitId, habit => {
     ...
     */
    const nextProgress = isCompleted ? fields.frequency : isFrequencyChanged ? 0 : habit.currentProgress;
    return {
      ...habit,
      ...fields,
  ...
  const currentIndex = nextHabits.findIndex(habit => habit.id === payload.habitId);
  const newIndex = Number(payload.data.order ?? 1) - 1;

  // Reorder if the position has changed
  if (currentIndex !== -1 && newIndex !== currentIndex) {
    nextHabits = reorderHabit({
      habits: nextHabits,
      newIndex,
      currentIndex
    });
