  ...
  getItem: key => {
    ...
    try {
      ...
      if (data?.state?.habits && isNotesMigrated) {
        data.state.habits = data.state.habits.map(h => {
          const nextHabit = {
            ...h
          };
 ...
 */
export const useHabitsStore = create()(persist(set => ({
  habits: [],
  habitsDispatch: action => set(s => ({
    habits: habitsReducer(s.habits, action)
  }))
}), {
